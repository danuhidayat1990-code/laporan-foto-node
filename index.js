...
app.get("/export/excel", async (req, res) => {
  ...
  for (let i = 0; i < laporan.length; i++) {
    ...
    try {
      // Ambil & kompres gambar dari Cloudinary
      const response = await axios.get(item.fotoURL, { responseType: "arraybuffer" });
      let buffer = Buffer.from(response.data, "binary");

      buffer = await sharp(buffer).resize(200).jpeg({ quality: 70 }).toBuffer();
      ...
      console.log("Gagal fetch image Excel:", err.message);
    }
  }
        ...
        children: await Promise.all(
          laporan.map(async (item, index) => {
            ...
            try {
              ...
              console.log("Gagal fetch image Word:", err.message);
            }
